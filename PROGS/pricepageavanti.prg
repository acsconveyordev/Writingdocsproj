...
*********************
* As of 12/08/04 a fork truck note was added thus it also must be deleted
* go to Paragraph 108+pcount
* WordApp.ActiveDocument.Paragraphs(108+pcount).Range.Select
* WordApp.Selection.Delete
* pcount = pcount - 2
*********************
...
WordApp.Selection.Delete
* insert revision
WordApp.Selection.InsertAfter(" installation, installation materials, controls, custom clearance or taxes")
WordApp.Selection.EndKey
* add a paragraph
...
WordApp.Selection.TypeParagraph
pcount = pcount + 1
WordApp.Selection.InsertAfter("CSC (Container Service Charge) is included in freight estimate.")
WordApp.Selection.EndKey
* add a paragraph
WordApp.Selection.TypeParagraph
pcount = pcount + 1
WordApp.Selection.InsertAfter("Freight Summary for base system:")
WordApp.Selection.EndKey
* add a paragraph
WordApp.Selection.TypeParagraph
pcount = pcount + 1
WordApp.Selection.InsertAfter(chr(9) + "( ) 40 Foot Container(s) @$ each = $")
WordApp.Selection.EndKey
* add a paragraph
WordApp.Selection.TypeParagraph
pcount = pcount + 1
WordApp.Selection.InsertAfter(chr(9) + "FF Fee @ $85 each = $")
WordApp.Selection.EndKey
* add a paragraph
WordApp.Selection.TypeParagraph
pcount = pcount + 1
WordApp.Selection.InsertAfter(chr(9) + "Insurance @ $0.60 per $100 Value = $" + chr(9) + "Total Freight Estimate: $")
* change "Total Freight Estimate: $" to Double UnderLine
*!*	WordApp.ActiveDocument.Paragraphs(108+pcount).Range.Select
WordApp.ActiveDocument.Paragraphs(103+pcount).Range.Select
WordApp.Selection.Find.ClearFormatting
With WordApp.Selection.Find
	.Text = "Total Freight Estimate: $"
	.Forward = .t.
	.Wrap = wdFindContinue
	.Format = .f.
	.MatchCase = .t.
...
if nnumofgroups # 1    && if 1 the table is removed thus no need to edit it
	...
	* adjust the width of the columns 3 and then 2
	...
	WordApp.Selection.Moveleft(wdCharacter,1)
	WordApp.Selection.SelectColumn
	WordApp.Selection.Tables(1).Columns(2).SetWidth(340,wdAdjustNone)
	WordApp.Selection.Collapse
endif
