* Name......... write the PRICE PAGE for WEYerhaeuser with OVERall discount not to be shown
* Date......... 11/04/2010
...
*!*			do numberformat with 3,"a.material + a.trnprice + a.pc_tech"
		do numberformat with 3,"a.material + a.pc_tech"
		do numberformat with 4,"a.install + a.rtotal"
...
if nnumofgroups = 1
	* table and following paragraph will be deleted later
	...
	WordApp.Selection.InsertAfter("Above Prices do not include corporate discount.")
endif
******
...
With MyTotalTable
	* the material and pc_tech to be added together
*!*		do numberformat with 3,"a.material + a.trnprice + a.pc_tech"
	do numberformat with 3,"a.material + a.pc_tech"
	tcount = tcount + 1
	* the installation and rtotal to be added together and discounted together
	do numberformat with 3,"a.install+ a.rtotal"
	tcount = tcount + 3
...
*!*					if a.material + a.trnprice + a.pc_tech < 0
				...
				WordApp.Selection.EndKey
*!*					do numberformatopt with "(a.material + a.trnprice + a.pc_tech)"
				do numberformatopt with "(a.material + a.pc_tech)"
			...
			* at his point the cursor is at the beginning of the line
			if a.install + a.rtotal = 0    && remove the line for install
				...
				* at this time the cursor is at the beginning of the ADD TOTAL line
				...
				store 0 to ndisinstallrrr
				WordApp.Selection.EndKey
				do numberformatopt with "a.install + a.rtotal"
				WordApp.Selection.MoveRight(wdCharacter,2)
...
*!*					if a.material + a.trnprice + a.pc_tech - g.material - g.trnprice - g.pc_tech < 0
				...
				store 0 to ndismaterialpc
				WordApp.Selection.EndKey
*!*					do numberformatopt with "(a.material + a.trnprice + a.pc_tech - g.material - g.trnprice - g.pc_tech)"
				do numberformatopt with "(a.material + a.pc_tech - g.material - g.pc_tech)"
				WordApp.Selection.MoveRight(wdCharacter,2)
				* cursor is at the beginning
			endif	   
			* at his point the cursor is at the beginning of the line
			if a.install + a.rtotal - g.install - g.rtotal = 0    && remove the line for install
				...
				* at this time the cursor is at the beginning of the ADD TOTAL line
				...
				store 0 to ndisinstallrrr
				WordApp.Selection.EndKey
				do numberformatopt with "(a.install + a.rtotal - g.install - g.rtotal)"
				WordApp.Selection.MoveRight(wdCharacter,2)
				* cursor is at the beginning of the ADD TOTAL line
			endif
...
*!*				   a.total - g.total = a.install - g.install + a.rtotal - g.rtotal
		...
		endif   && (a.material + a.pc_tech - g.material - g.pc_tech ) = 0 .and. (a.install + a.rtotal - g.install - g.rtotal) = 0
...
do while eof() = .f.
	if eof() = .f.
		...
		* paragraph 111
*!*			WordApp.ActiveDocument.Paragraphs(120+pcount).Range.Select
		WordApp.ActiveDocument.Paragraphs(111+pcount).Range.Select
...
*!*				   a.rtotal - g.rtotal = 0 .and. a.trnprice - g.trnprice = 0 .and. a.pc_tech - g.pc_tech = 0			   
			if a.material - g.material = 0 .and. a.install - g.install = 0 .and. ;
			   a.rtotal - g.rtotal = 0 .and. a.pc_tech - g.pc_tech = 0					&& do nothing
			else
*!*					if a.material - g.material + a.trnprice - g.trnprice + a.pc_tech - g.pc_tech = 0
...
*!*						if a.material - g.material + a.trnprice - g.trnprice + a.pc_tech - g.pc_tech < 0
					...
					WordApp.Selection.EndKey
					do numberformatopt with "(a.material - g.material + a.pc_tech - g.pc_tech)"
					WordApp.Selection.EndKey
				endif
				if a.install - g.install + a.rtotal - g.rtotal = 0    && don't add the line for installation
					...
					* add information
					...
					store 0 to ndisinstallrrr
					WordApp.Selection.EndKey
					do numberformatopt with "(a.install - g.install + a.rtotal - g.rtotal)"
				endif
				WordApp.Selection.EndKey
				WordApp.Selection.TypeText(Chr(11) + Chr(9) + Chr(9))
				* a.total - g.total = a.material - g.material + a.pc_tech - g.pc_tech .or. ;
