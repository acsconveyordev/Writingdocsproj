* Name......... WRITE the OPTion and SHOW DISCount 01 information from foxpro into word program
* Date......... 10/22/2010
* Caller....... See Notes
		...
		* move cursor to the beginning of the Add Material statement
		...
		if a.material = 0    && remove the line for material
			...
			* cursor is at the beginning
			...
			case ncompnumber = 546
				if a.material < 0
					...
					WordApp.Selection.InsertBefore("Deduct List")
				else
					WordApp.Selection.MoveRight(wdCharacter,4)
			...
			* show discount
			...
			if ndismaterial < 0
				WordApp.Selection.TypeText(Chr(9)+Chr(9)+"Less " + ltrim(cdisc) + "% Material Discount"+Chr(9)+Chr(9))
			else
				WordApp.Selection.TypeText(Chr(9)+Chr(9)+"Add " + ltrim(cdisc) + "% Material Discount"+Chr(9)+Chr(9))
			endif
			do numberformatopt with "ndismaterial"
			WordApp.Selection.Font.Italic = .f.
			...
			do numberformatopt with "nnetmaterial"
			WordApp.Selection.EndKey
			WordApp.Selection.MoveLeft(wdCharacter,1)
			WordApp.Selection.TypeText(Chr(11))
			* add an empty line
		...
		* move cursor to the beginning of the Add Material statement
		...
		if a.material = 0    && remove the line for material
			...
			* cursor is at the beginning
			...
			do case
			case ncompnumber = 546 .and. nmaterialdisc > 0
			...
			* show discount
			store str(nmaterialdisc * 100,5,2) to cdisc
			store round((a.material - g.material) * nmaterialdisc,0) * -1 to ndisrefmaterial
			WordApp.Selection.Font.Italic = .t.
			if ndisrefmaterial < 0
				WordApp.Selection.TypeText(Chr(9)+Chr(9)+"Less " + ltrim(cdisc) + "% Material Discount"+Chr(9)+Chr(9))
			else
				WordApp.Selection.TypeText(Chr(9)+Chr(9)+"Add " + ltrim(cdisc) + "% Material Discount"+Chr(9)+Chr(9))
			endif
			do numberformatopt with "ndisrefmaterial"
			WordApp.Selection.Font.Italic = .f.
			WordApp.Selection.Font.Underline = wdUnderlineSingle
			WordApp.Selection.EndKey
			WordApp.Selection.MoveLeft(wdCharacter,1)
			WordApp.Selection.TypeText(Chr(11))
			WordApp.Selection.Font.Underline = wdUnderlineNone
			store (a.material - g.material) + ndisrefmaterial to nnetrefmaterial
			if nnetrefmaterial < 0
				WordApp.Selection.TypeText(Chr(9)+Chr(9)+"Deduct Net Material"+Chr(9)+Chr(9))
			else
				WordApp.Selection.TypeText(Chr(9)+Chr(9)+"Add Net Material"+Chr(9)+Chr(9))
			endif
			do numberformatopt with "nnetrefmaterial"
			WordApp.Selection.EndKey
			WordApp.Selection.MoveLeft(wdCharacter,1)
			WordApp.Selection.TypeText(Chr(11))
			* add an empty line
		...
		* don't print this line
		if a.total - g.total = a.material - g.material .or. a.total - g.total = a.install - g.install .or. ;
			...
			* at this time the cursor is at the beginning of the ADD TOTAL line
			...
			do case
			case ncompnumber = 546 .and. (nmaterialdisc > 0 .or. ninstalldisc > 0)
			...
			endcase
			WordApp.Selection.EndKey
			do numberformatopt with "a.total - g.total + ndisrefmaterial - ndisinstall - ndiscr"
